...
- `INTERNET` and `ACCESS_NETWORK_STATE` for Groq requests;
- `VIBRATE` for keyboard haptic feedback;
...
This covers the Kotlin/Gradle source and XML resources authored for this
project. It does **not** independently verify provenance for the bundled
image and animation assets (`app/play_store_assets/`, `play-store/`,
`icons/`, launcher mipmaps, `res/raw/keyboard_setup_animation.json`) — several
were generated by prior development agents per `LOGBOOK.md` and are included
as-is without a separate asset audit. If you reuse this repository, replace
or independently verify those assets before redistributing them.

Sensum depends on third-party open-source libraries (Retrofit, OkHttp, Moshi,
AndroidX, Material Components, Lottie, WorkManager, and others declared in
`gradle/libs.versions.toml`), each under its own licence (mostly Apache
License 2.0). Those libraries are not relicensed by this project; consult
each dependency's own repository for its terms.
